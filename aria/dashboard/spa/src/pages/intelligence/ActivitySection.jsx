  ...
  return (
    <div class="space-y-2">
      <div class="flex justify-between items-baseline">
        <div class="text-xs font-medium" style="color: var(--text-secondary)">Activity Timeline (6h)</div>
        <div class="text-xs" style="color: var(--text-tertiary)">{totalEvents} events, home {pctOccupied}% of the time</div>
      </div>
...
function WsHealthIndicator({ ws }) {
  ...
  return (
    <div class="flex items-center gap-2 text-xs">
      <span class="inline-block w-2.5 h-2.5 rounded-full" style={`background: ${connected ? 'var(--status-healthy)' : 'var(--status-error)'}`} />
      <span style={`color: ${connected ? 'var(--status-healthy)' : 'var(--status-error)'}`}>
        {connected ? 'Connected' : 'Disconnected'}
      </span>
      {ws.disconnect_count > 0 && (
        <span style="color: var(--text-tertiary)">({ws.disconnect_count} reconnect{ws.disconnect_count !== 1 ? 's' : ''})</span>
      )}
      {ws.total_disconnect_s > 0 && (
        <span style="color: var(--text-tertiary)">{Math.round(ws.total_disconnect_s)}s downtime</span>
      )}
    </div>
  );
}
...
export function ActivitySection({ activity }) {
  if (!activity) {
    return (
      ...
      >
        <Callout>Activity monitoring is starting up. State changes will appear here once the WebSocket connection is established.</Callout>
      </Section>
    );
  }
    ...
    <Section title="Activity Monitor" subtitle={contextSubtitle} summary={occ.anyone_home ? "Home" : "Away"}>
      <div class="space-y-4">
        ...
        <div class="flex items-center justify-between">
          ...
          {eventPredictions.predicted_next_domain && (
            <div class="text-xs" style="color: var(--text-tertiary)">
              Next likely: <span class="font-medium" style="color: var(--text-secondary)">{DOMAIN_LABELS[eventPredictions.predicted_next_domain] || eventPredictions.predicted_next_domain}</span>
        ...
        {anomalies.length > 0 && anomalies.map((a, i) => (
          <Callout key={i}>{a.message}</Callout>
        ))}
        ...
        <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div class="t-frame p-4" style={`border-left: 4px solid ${occ.anyone_home ? 'var(--status-healthy)' : 'var(--border-subtle)'}`}>
            ...
            <div class="text-sm mt-1" style="color: var(--text-tertiary)">Occupancy</div>
            {occ.anyone_home && occ.since && (
              <div class="text-xs mt-1" style="color: var(--text-tertiary)">for {durationSince(occ.since)}</div>
            )}
            {!occ.anyone_home && occPrediction.predicted_arrival && occPrediction.status === 'predicted' && (
              <div class="text-xs mt-1" style="color: var(--accent)">
                ...
                <span style="color: var(--text-tertiary)"> ({occPrediction.confidence}, {occPrediction.based_on} samples)</span>
              </div>
            )}
          ...
          <div class="t-frame p-4">
            ...
            {rate.avg_1h > 0 && (
              <div class="text-xs mt-1" style={`color: ${
                rate.trend === 'increasing' ? 'var(--status-warning)' :
                rate.trend === 'decreasing' ? 'var(--accent)' : 'var(--text-tertiary)'
              }`}>
          ...
          <div class="t-frame p-4">
            ...
            {snap.cooldown_remaining_s > 0 && (
              <div class="text-xs mt-1" style="color: var(--text-tertiary)">Next eligible in {Math.ceil(snap.cooldown_remaining_s / 60)}m</div>
            )}
          </div>
        </div>
                ...
                {recentEvents.map((evt, i) => {
                  ...
                  return (
                    <div key={evtKey} class="flex items-center gap-2 py-1 px-1 rounded" style={isSignificant ? 'background: var(--status-warning-glow)' : ''}>
                      ...
                      <span class="text-xs flex-shrink-0" style="color: var(--text-tertiary)">{evt.time}</span>
                    </div>
                  );
                })}
              </div>
                    ...
                    return (
                      <div key={domain} class="space-y-0.5">
                        ...
                        <div class="h-1.5 rounded-full" style="background: var(--bg-inset)">
                          <div class="h-1.5 rounded-full" style={`background: var(--accent-dim); width: ${pct}%`} />
                        </div>
                      </div>
                    );
                  });
              ...
              {patterns.map((p, i) => (
                <div key={i} class="flex items-center gap-2 text-sm">
                  <div class="flex items-center gap-1">
                    {p.sequence.map((s, j) => (
                      <span key={j}>
                        <span class="rounded px-1.5 py-0.5 text-xs font-medium" style="background: var(--accent-glow); color: var(--accent)">
                          {DOMAIN_LABELS[s] || s}
                        </span>
                        {j < p.sequence.length - 1 && <span class="mx-0.5" style="color: var(--text-tertiary)">&rarr;</span>}
                      </span>
                    ))}
                  </div>
                  <span class="text-xs ml-auto flex-shrink-0" style="color: var(--text-tertiary)">{p.count}x{p.last_seen ? `, last ${p.last_seen.slice(11, 16) || p.last_seen}` : ''}</span>
                </div>
              ))}
            </div>
          </div>
                ...
                <tbody>
                  {snap.log_today.map((entry, i) => (
                    <tr key={i} style="border-bottom: 1px solid var(--border-subtle)">
                      <td class="px-4 py-1.5" style="color: var(--text-tertiary)">{entry.number}</td>
                      <td class="px-4 py-1.5" style="color: var(--text-secondary)">{entry.timestamp?.slice(11, 16)}</td>
                      <td class="px-4 py-1.5">{entry.buffered_events} buffered</td>
                      <td class="px-4 py-1.5" style="color: var(--text-secondary)">{(entry.people || []).join(', ') || '\u2014'}</td>
                      <td class="px-4 py-1.5">
                        <div class="flex flex-wrap gap-1">
                          {Object.entries(entry.domains || {}).slice(0, 4).map(([d, c]) => (
                            <span key={d} class="rounded px-1.5 py-0.5 text-xs" style="background: var(--bg-surface-raised)">
                              {DOMAIN_LABELS[d] || d}: {c}
                            </span>
                          ))}
                        </div>
